...
class ChatMemberRepositoryTest extends AbstractContainerTest {
  ...
  void setUp() {
    // 1. 부모 엔티티(User) 저장
    User testUser =
        User.builder().nickname("수영구보안관").name(new Name("조", "준연")).email("test@test.com").build();
    em.persist(testUser);
    userId = testUser.getId();

    // 2. 부모 엔티티(LocalChatroom) 저장
    // 💡 NOT NULL 제약조건이 걸린 max_members와 필수 값들을 채워줍니다.
    LocalChatroom testRoom =
    ...
    ChatMember chatMember = ChatMember.builder().chatroom(testRoom).user(testUser).build();
